...
@IBDesignable class UnderlineSegmentedControl: UIControl {
    ...
    override func layoutSubviews() {
        ...
        thumbView.frame = CGRect(x: selectFrame.origin.x+(selectFrame.size.width/3), y: selectFrame.size.height-1, width: selectFrame.size.width/3, height: 1)//selectFrame
    ...
    func displayNewselectedSegmentIndex(){
        ...
        UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.8, options: [], animations: {
            ...
            self.thumbView.frame = CGRect(x: label.frame.origin.x+(label.frame.size.width/3.5), y: label.frame.size.height-1, width: label.frame.size.width/2.5, height: 1)
